...
const Sparkles = () => {
  // Randomness lives in an effect (impurity is not allowed during render); the
  // stars scatter once on mount, then drift on their own loops.
  const [stars, setStars] = React.useState<Star[]>([])

  useEffect(() => {
    const rnd = () => Math.random()
    // Mount-only: scatter the decorative stars once after the client mounts.
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setStars(
      Array.from({ length: 12 }).map(() => ({
        top: rnd() * 100,
        left: rnd() * 100,
        animateTop: `calc(${rnd() * 100}% + ${rnd() * 2 - 1}px)`,
        animateLeft: `calc(${rnd() * 100}% + ${rnd() * 2 - 1}px)`,
        opacity: rnd(),
        duration: rnd() * 2 + 4,
      }))
    )
